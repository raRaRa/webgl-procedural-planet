...
/**
 * Subtracts a vec3 by a scalar number
 *
 * @param {vec3} out the receiving vector
...
/**
 * Subtracts a vec4 by a scalar number
 *
...
function permute_float_v2(x) {
    return mod289_float_v2( ((x*34.0)+1.0)*x );
}
...
var lhs_v2 = vec4.fromValues(1.79284291400159, 1.79284291400159, 1.79284291400159, 1.79284291400159);

function taylorInvSqrt_vec4_v2(r) {

    var temp = vec4.create();
    temp = vec4.scale(temp, r, 0.85373472095314);
    temp = vec4.sub(temp, lhs_v2, temp);
...
function taylorInvSqrt_float_v2(r) {
    return 1.79284291400159 - 0.85373472095314 * r;
}

function fract_vec3_v2(x) {
    var temp = vec3.create();

    return vec3.sub(temp, x, vec3.floor(temp, x))
}

function fract_float_v2(x) {
    return x - Math.floor(x);
}

var ip_v2 = vec4.fromValues(1.0/294.0, 1.0/49.0, 1.0/7.0, 0.0);
var ones_v2 = vec3.fromValues(1.0, 1.0, 1.0);

function grad4_v2(j) {
   //    var ones_v2 = Vec4.fromValues(1.0, 1.0, 1.0, -1.0);
    //var p = Vec4.create();
    //var s = Vec4.create();
    //p.xyz = floor( fract (vec3(j) * ip_v2.xyz) * 7.0) * ip_v2.z - 1.0;
    var temp = vec3.create();
    temp = vec3.mul(temp, vec3.fromValues(j, j, j), vec3.fromValues(ip_v2[0], ip_v2[1], ip_v2[2]));
    temp = fract_vec3_v2(temp);
    temp = vec3.scale(temp, temp, 7.0);
    temp = vec3.floor(temp, temp);
    temp = vec3.mul(temp, temp, vec3.fromValues(ip_v2[2], ip_v2[2], ip_v2[2]));
    temp = vec3.subScalar(temp, temp, 1.0);

    //p.w = 1.5 - dot(abs(p.xyz), ones_v2.xyz);
    var temp2 = vec3.create();
    temp2 = vec3.abs(temp2, vec3.fromValues(temp[0], temp[1], temp[2]));
    temp2 = 1.5 - vec3.dot(temp2, ones_v2);
...
//float snoise_v2(vec4 v)
function snoise_v2(v) {
    ...
    var temp1 = Vec3.sub(ones_v2, isX);
    i0.y = temp1.x;
    i0.z = temp1.y;
    i0.w = temp1.z;
    i0.y += isYZ.x + isYZ.y;
    ...
    vec4 i3 = clamp_v2( i0, 0.0, 1.0 );
    vec4 i2 = clamp_v2( i0-1.0, 0.0, 1.0 );
    vec4 i1 = clamp_v2( i0-2.0, 0.0, 1.0 );*/

    var i3 = Vec4.clamp_v2(i0, 0.0, 1.0);
    var i2 = Vec4.clamp_v2( Vec4.subNum(i0, 1.0), 0.0, 1.0);
    var i1 = Vec4.clamp_v2( Vec4.subNum(i0, 2.0), 0.0, 1.0);
...
//.log    console.log(i);
    ...
    var j0 = permute_float_v2( permute_float_v2( permute_float_v2( permute_float_v2(i.w) + i.z) + i.y) + i.x);
  ...
  /*  vec4 j1_tmp1 = permute( vec4(i.w, i.w, i.w, i.w) + vec4(i1.w, i2.w, i3.w, 1.0 ) );
    ...
    var j1_tmp1 = permute_vec4_v2( Vec4.addNum(Vec4.fromValues(i1.w, i2.w, i3.w, 1.0 ), i.w) );
    var j1_tmp2 = permute_vec4_v2( Vec4.add(Vec4.addNum(j1_tmp1, i.z), Vec4.fromValues(i1.z, i2.z, i3.z, 1.0) ));
    var j1_tmp3 = permute_vec4_v2( Vec4.add(Vec4.addNum(j1_tmp2, i.y), Vec4.fromValues(i1.y, i2.y, i3.y, 1.0) ));
    var j1 = permute_vec4_v2( Vec4.add(Vec4.addNum(j1_tmp3, i.x), Vec4.fromValues(i1.x, i2.x, i3.x, 1.0) ));

    // Gradients: 7x7x6 points over a cube, mapped onto a 4-cross polytope
    // 7*7*6 = 294, which is close to the ring size 17*17 = 289.
    /*vec4 ip_v2 = vec4(1.0/294.0, 1.0/49.0, 1.0/7.0, 0.0) ;

    vec4 p0 = grad4_v2(j0,   ip_v2);
    vec4 p1 = grad4_v2(j1.x, ip_v2);
    vec4 p2 = grad4_v2(j1.y, ip_v2);
    vec4 p3 = grad4_v2(j1.z, ip_v2);
    vec4 p4 = grad4_v2(j1.w, ip_v2);*/

    
    var p0 = grad4_v2(j0);
    var p1 = grad4_v2(j1.x);
    var p2 = grad4_v2(j1.y);
    var p3 = grad4_v2(j1.z);
    var p4 = grad4_v2(j1.w);
   ...
   /* vec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));
    ...
    var norm = taylorInvSqrt_vec4_v2(Vec4.fromValues(Vec4.dot(p0,p0), Vec4.dot(p1,p1), Vec4.dot(p2, p2), Vec4.dot(p3,p3)));
    p0 = Vec4.scale(p0, norm.x);
    p1 = Vec4.scale(p1, norm.y);
    p2 = Vec4.scale(p2, norm.z);
    p3 = Vec4.scale(p3, norm.w);
    p4 = Vec4.scale( p4, taylorInvSqrt_float_v2(Vec4.dot(p4,p4)) );
...
/*
float fBm3_v2(in vec3 p, in float H, in float lacunarity, in float offset, in float gain, float time) {
    float result; 
    float frequency = 1.0; 
    float signal;
    float weight;

    signal = offset - abs(snoise_v2(vec4(p, time)));
    signal*= signal;
    result = signal;
    weight = 1.0;
    ...
    for(int i = 0; i < octaves; i++) {
        p*= lacunarity;
        weight = clamp_v2(signal*gain, 0.0,1.0);
        signal = offset - abs(snoise_v2(vec4(p, time)));
...
function fBm3_v2(p, H, lacunarity, offset, gain, time) {
    var result; 
    var frequency = 1.0; 
    var signal;
    var weight;
    ...
    signal = offset - Math.abs(snoise_v2(Vec4.fromValues(p.x, p.y, p.z, time)));
    signal*= signal;
    result = signal;
    weight = 1.0;
    ...
    for(var i = 0; i < octaves; i++) {
        ...
        weight = clamp_v2(signal*gain, 0.0, 1.0);
        signal = offset - Math.abs(snoise_v2(Vec4.fromValues(temp.x, temp.y, temp.z, time)));
        signal *= signal * weight;
...
/*
float getTerraced_v2(float val, float n, float power)
{
    float dVal = val * n;
    float f = fract(dVal);
    float i = floor(dVal);
...
function getTerraced_v2(val, n, power)
{
    var dVal = val * n;
    var f = fract_float_v2(dVal);
